...
int _rm_disp32(u32 off, u8 *reg) {
    ...
    /* 4-byte displacement */
    if(mod == 2 || (mod == 0 && rm == 5) || (mod == 0 && rm == 4 && (core[off + 1] & 0x7) == 5))
        return 1;
...
static label_type_t _vtable_access(u32 off) {
     ...
     */
    if(b == 0xf && core[off + 1] == 0xb6 && _rm_disp32(off + 2, NULL))
        return label_DATA;
...
void dis_pass1(void) {
    ...
    for(i = 0; segment[i].name != NULL; i++) {
        if(segment[i].type != seg_CODE) continue;

        printf("dis1: processing segment '%s' (size 0x%x)\n", segment[i].name, segment[i].size);

        /* find the first relocation in this segment */
        for(; ir < nreloc; ir++)
            if(reloc[ir].off >= segment[i].coff)
                break;
        ...
        while(off < segment[i].coff + segment[i].size) {
            ...
            if(ir < nreloc && off + len > reloc[ir].off && reloc[ir].off >= segment[i].coff && reloc[ir].off < segment[i].cend) {
            ...
            /* progress report */
            if(verbose)
                if(((off - segment[i].coff) & 0xfffff000) != ((off - segment[i].coff + len) & 0xfffff000))
                    printf("  processed 0x%x bytes\n", off + len - segment[i].coff);
        }
    }
